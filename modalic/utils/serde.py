...
def weights_to_parameters(weights: protocol.Weights, dtype: str, model_version: int) -> protocol.Parameters:
    ...
    return protocol.Parameters(tensor=tensor, data_type=dtype, model_version=model_version)
...
def parameters_to_weights(parameters: protocol.Parameters, shapes) -> protocol.Weights:
    ...
    return bytes_to_ndarray(parameters.tensor, shapes, dtype_to_struct(parameters.data_type))
